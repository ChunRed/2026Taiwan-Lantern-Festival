...
<template>
  <div class="Layer absolute w-full h-full left-0 top-0 z-0">
    ...
    <div
      class="absolute inset-x-0 top-0 h-screen sticky pointer-events-none z-0 overflow-hidden"
    >
      <!-- Top Left Image -->
      <img
        :src="getImageUrl(currentData.image)"
        class="icon1 absolute top-14 -left-10 w-[250px] rotate-180"
        alt=""
      />
      ...
      <img
        :src="getImageUrl(currentData.image)"
        class="icon2 absolute top-40 left-48 w-[250px]"
        alt=""
      />
    </div>
  </div>
